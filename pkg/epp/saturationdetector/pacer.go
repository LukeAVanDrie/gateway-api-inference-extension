...
// TimeUntilReady returns the duration until the token bucket has enough tokens to cover the specified cost.
// Returns 0 if currently allowed.
// Returns Infinite if rate is 0.
func (p *Pacer) TimeUntilReady(cost float64) time.Duration {
	if cost <= 0 {
		return 0
	}
	...
	// We need 'deficit' tokens.
	deficit := cost - p.tokens
