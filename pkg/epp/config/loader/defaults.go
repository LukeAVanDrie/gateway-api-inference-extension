...
func applySystemDefaults(cfg *configapi.EndpointPickerConfig, handle plugins.Handle) error {
	...
	// Ensure the critical flow control components (Saturation Controller) exist.
	if err := ensureFlowControlArchitecture(cfg, handle, allPlugins); err != nil {
...
// ensureFlowControlArchitecture guarantees that a Saturation Controller is present.
func ensureFlowControlArchitecture(
	cfg *configapi.EndpointPickerConfig,
	handle plugins.Handle,
	allPlugins map[string]plugins.Plugin,
) error {
	...
	// Scan for existing instances of the Saturation Controller.
	for name, p := range allPlugins {
		if _, ok := p.(satctrl.SaturationController); ok {
			if existingName != "" {
				return fmt.Errorf("multiple saturation controllers found ('%s', '%s'); exactly one is required",
					existingName, name)
			}
			existingName = name
		}
	...
	// If found, we are satisfied.
	if existingName != "" {
		return nil
	}

	// If not found, instantiate the default via the Registry.
	return registerDefaultPlugin(
		cfg,
		handle,
		staticthreshold.StaticThresholdSaturationControllerType,
		staticthreshold.StaticThresholdSaturationControllerType,
	)
}
