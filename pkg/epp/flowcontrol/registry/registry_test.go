...
func newRegistryTestHarness(t *testing.T, opts harnessOptions) *registryTestHarness {
	...
	handle := &fakeHandle{}
	fr, err := NewFlowRegistry(*validatedCfg, logr.Discard(), handle, registryOpts...)
...
func TestFlowRegistry_New(t *testing.T) {
	...
	t.Run("ShouldFail_WhenInterFlowPluginNotFound", func(t *testing.T) {
		t.Parallel()
		...
		require.Error(t, err, "ValidateAndApplyDefaults should fail")
		assert.Contains(t, err.Error(), "non-existent-policy", "Error message should mention the missing policy")
		assert.Contains(t, err.Error(), "not found in plugin registry", "Error message should indicate registry lookup failure")
	})

	t.Run("ShouldFail_WhenInitialShardCreationFails_DueToFactoryError", func(t *testing.T) {
		...
		defer delete(plugins.Registry, policyName) // Cleanup

		validatedCfg, err := config.ValidateAndApplyDefaults()
		require.NoError(t, err, "Validation should pass as factory exists")

		_, err = NewFlowRegistry(*validatedCfg, logr.Discard(), &fakeHandle{})
