...
* **Request Distribution**: **Note: The current MVP implementation uses a single worker shard, so no distribution is
  required.** The target architecture for multi-shard support uses a top-level distributor that receives requests and
  uses a **Join-the-Shortest-Queue-by-Bytes (JSQ-Bytes)** algorithm to assign each request to a specific worker shard,
...
* **Direct Backpressure Propagation**: If a queue is full, `EnqueueAndWait` returns an `ErrQueueAtCapacity`. This
  provides immediate, direct backpressure to the earliest point of contact, preventing the system from accepting work it
  cannot handle.
...
**Note: The current MVP implementation uses a single shard. The following sections describe the target architecture.**

### The Sharded Architecture and its Implications

In the target architecture, the `FlowController` consists of a top-level manager and a pool of independent
`shardProcessor` workers. The `FlowRegistry` guarantees that every logical flow is represented by a distinct queue
instance on every active shard. This creates `N` parallel instances for each flow, managed by `N` independent workers.
...
To make the critical assumption as robust as possible, the target `FlowController` architecture uses a
