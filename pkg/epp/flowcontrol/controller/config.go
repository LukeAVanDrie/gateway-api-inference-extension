...
// Config holds the top-level configuration for a `FlowController` instance.
type Config struct {
	// DefaultQueueTTL is the default Time-To-Live applied to requests within queues if the incoming request does not
	// specify its own via `types.FlowControlRequest.InitialEffectiveTTL()`.
	//
...
// TODO: Add ValidateAndApplyDefaults...
