...
// This allows for efficient retrieval of the maximum priority item.
...
func (h *maxMinHeap) PeekTail() (types.QueueItemAccessor, error) {
	...
	// With three or more items, the minimum element is one of the two children of the root.
	// We must compare them to find the true minimum.
...
func (h *maxMinHeap) Remove(handle types.QueueItemHandle) (types.QueueItemAccessor, uint64, uint64, error) {
	...
	if i < n {
		...
		// The swapped item at index i might violate the heap property.
		// We first try to bubble it down, and if that doesn't move it, we try to bubble it up.
		h.down(i)
	} else {
...
func (h *maxMinHeap) Cleanup(predicate framework.PredicateFunc) ([]types.QueueItemAccessor, error) {
	...
	if len(removedItems) > 0 {
		h.items = itemsToKeep
		// Re-heapify the remaining items.
		// This is done by rebuilding the heap from the modified slice.
		// First, update all the indices in the handles map.
		for i, item := range h.items {
			h.handles[item.Handle()].index = i
		}
		// Then, starting from the last non-leaf node, trickle down.
