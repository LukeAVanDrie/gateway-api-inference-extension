...
// SelectItem selects the next item from the queue by peeking its head.
// It assumes the queue itself is FIFO ordered.
...
// Comparator returns an ItemComparator based on enqueue time.
...
func (c *enqueueTimeComparator) ScoreType() string {
	return string(dispatch.EnqueueTimePriorityScoreType)
}
