...
//
// While a simpler boolean method (e.g., `IsPriorityConfigurable()`) could satisfy current needs,
// this slice-based approach is intentionally chosen for future extensibility. It allows a queue
// to advertise multiple features (e.g., batching, size bounds) without future breaking changes
// to the SafeQueue interface.
type QueueCapability string

const (
	// CapabilityFIFO indicates that the queue operates in a First-In, First-Out manner.
	// `PeekHead()` will return the oldest item.
...
// QueueInspectionMethods defines common read-only methods shared by `SafeQueue` and `FlowQueueAccessor`.
type QueueInspectionMethods interface {
	...
	// PeekHead returns the item at the "head" of the queue (according to the queue's ordering) without removing it.
	//
	// Returns `ErrQueueEmpty` if the queue is empty.
	PeekHead() (peekedItem types.QueueItemAccessor, err error)

	// PeekTail returns the item at the "tail" of the queue (the item with the lowest priority) without removing it.
	//
	// Returns `ErrQueueEmpty` if the queue is empty.
