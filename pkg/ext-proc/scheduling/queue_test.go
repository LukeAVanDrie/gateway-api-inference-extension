...
func TestFairRequestQueue(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			frq, err := NewFairRequestQueue(test.totalCapacity, test.flowCapacity)
			if err != nil {
			...
			for i := 0; i < len(test.requests); i++ {
				...
				req := tsr.queuedRequestContext.(*fakeRequest)
...
func TestFairRequestQueue_Concurrent(t *testing.T) {
	...
	frq, _ := NewFairRequestQueue(totalCapacity, flowCapacity)

	var startWg, pushWg, popWg sync.WaitGroup
	pushersDone := make(chan struct{})
	poppedRequests := make(chan *timestampedQueuedRequestContext, numRequests)
	...
	for tsr := range poppedRequests {
		fr := tsr.queuedRequestContext.(*fakeRequest)
	...
	if frq.virtualTime != numRequests {
...
func TestFairRequestQueue_ProportionalService(t *testing.T) {
	...
	var startWg, pushWg, popWg sync.WaitGroup
	pushersDone := make(chan struct{})
	poppedRequests := make(chan *timestampedQueuedRequestContext, popLimit)
	testDone := make(chan struct{}) // Signals test completion/failure
	defer close(testDone)
...
// (collectively).
func runConcurrentPops(t *testing.T, frq *FairRequestQueue, popWg, startWg *sync.WaitGroup, poppedRequests chan *timestampedQueuedRequestContext, testDone, pushersDone chan struct{}, popLimit, numPoppers int) {
	t.Helper()
